...
const showProductDetailsInModal = (product_details) => {
   console.log(product_details.description);
...
const updateTaxAndCharge = () => {
   const priceCon = parseFloat(getInputValue('price'));
   const priceConverted = priceCon.toFixed(2)
   ...
   if (priceConverted > 200) {
      ...
      setInnerText1('total-tax', priceConverted * 0.2);
   }
   if (priceConverted > 400) {
      setInnerText('delivery-charge', 50);
      setInnerText1('total-tax', priceConverted * 0.3);
   }
   if (priceConverted > 500) {
      setInnerText('delivery-charge', 60);
      setInnerText1('total-tax', priceConverted * 0.4);
   }
};
...
const updateTotal = () => {
   ...
   const first = getInputValue('price') ;
   const second= getInputValue('delivery-charge')
   const third = getInputValue('total-tax');
   const grandTotal = first+second+third;
...
document.getElementById("search-btn").addEventListener("click", function () {
   ...
   const searchedProduct = arr[0].filter((p) =>
      p.title.toUpperCase().includes(`${inputField}`)
